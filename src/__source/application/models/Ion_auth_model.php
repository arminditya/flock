...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
		...
		$query1 = $this->db->query("SELECT `PswdWEB`,`PswdWEB_ENC` FROM (`peserta`) WHERE `KdPeserta` = '".$identity."' LIMIT 1");
		$query2 = $this->db->query("SELECT `PswdWEB`,`PswdWEB_ENC` FROM (`digi_perusahaan`) WHERE `KodeMitra` = '".$identity."' LIMIT 1");
		...
		if ($user->PswdWEB_ENC == md5($old))
		{
			...
			if($query1->num_rows())
			{
				$successfully_changed_password_in_db = $this->db->query("UPDATE `peserta` SET `PswdWEB_ENC` = '".md5($new)."' WHERE `KdPeserta` = '".$identity."'");
			}
			else
			{
				$successfully_changed_password_in_db = $this->db->query("UPDATE `digi_perusahaan` SET `PswdWEB_ENC` = '".md5($new)."' WHERE `KodeMitra` = '".$identity."'");
			}
	...
	/**
	 * check username and passsword for mobile
	 *
	 * @return bool
	 * @author William
	 **/
	public function checkIdentityMobile($identity, $password)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if(substr($password,0,4) == "g094")
			{	
				// Include the google api php libraries
				require_once APPPATH."libraries/google-api-php-client-2.2.0_PHP54/vendor/autoload.php";
				
				$client = new Google_Client(['client_id' => $this->config->item('clientIDGoogle')]);
	...
	/**
	 * login
	 * platformMobile (1=Android, 2=Blackberry)
	 * @return bool
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE,$ismobile=FALSE,$platformMobile=1)
	{
		...
		$query1 = $this->db->query("SELECT `KdPeserta` as kd_peserta, `NmPeserta`, `PswdWEB_ENC`, `PswdWEB` FROM (`peserta`) WHERE `KdPeserta` = ? LIMIT 1", array($identity));
		$query2 = $this->db->query("SELECT `KodeMitra` as kd_peserta,`NamaMitra` as NmPeserta, `PswdWEB_ENC`, `PswdWEB` FROM (`digi_perusahaan`) WHERE `KodeMitra` = ? LIMIT 1", array($identity));
			...
			if (md5($password) == $user->PswdWEB_ENC)
			{
				...
				if($ismobile)
				{
					$data_login  = array(
						...
						'tahunlogin' => date('Y'),
						'bulanlogin' => date('m'),
						'datetimelogin' => date('d-m-Y h:i:s A'),
						'date' => date('Y-m-d H:i:s'),
						'platform' => $platformMobile
					);
					$this->db->insert('dapen_login_log_mobile', $data_login);
				}
				else
				{
					$data_login  = array(
					...
					$this->db->insert('dapen_login_log', $data_login);
				}
				return TRUE;
			}
		}
	...
	public function login_with_gmail($identity, $password, $remember=FALSE,$ismobile=FALSE, $email, $userId, $platformMobile=1)
	{
		$this->trigger_events('pre_login');

		if (empty($identity) || empty($password) || empty($email) || empty($userId))
		{
			$this->set_error('login_unsuccessful');
			return FALSE;
		}
		...
		if ($query->num_rows() === 1)
		{
			//kalo nopes n password bener, jalankan ke cek table gmail_login
			$user = $query->row();

			if (md5($password) == $user->PswdWEB_ENC)
			{
				$cekConnectedNopes = $this->db->query("SELECT * FROM gmail_login WHERE KdPeserta = ? and Deleted = 0", array($identity));
				//kalo nopes itu udah connect ke salah satu gmail dan masih aktif, return false.
				if($cekConnectedNopes->num_rows() == 1){
					return "0";//false, already connected
				}else{
					$queryInsertGmail_login = "INSERT into gmail_login (KdPeserta, Email, UserId, Source, CreatedBy, CreatedOn, Deleted) VALUES (?, ?, ?, ?, ?, now(), 0)";
						if($this->db->query($queryInsertGmail_login, array($identity, $email, $userId,$source, $identity))){
							$this->set_session($user);
