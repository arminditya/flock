...
class AturRole extends CI_Controller {
    ...
    {		
		if($this->session->userdata('logged_in')){
			if(page_check_authorized('Manage Role'))
			{
	...
	public function _aturrole()
    {
		...
		$crud->set_table('mst_role');
        $crud->where('mst_role.DELETED','0');
        ...
        $crud->add_fields('NAMA_ROLE');//Fields untuk menambahkan data baru
        $crud->edit_fields('NAMA_ROLE');

        $crud->display_as('NAMA_ROLE','Role Name');

        //Atur rule
        $crud->set_rules('NAMA_ROLE','Role Name','trim|required');
    ...
    function _outputview($output = null)
    {
        ...
        $this->template_lib->load('default','content/sisantuyadmin/aturrole_view',$data);
    }
	
	function _delete_role($primary_key){
		return $this->db->update('mst_role',array('DELETED' => '1','UPDATED_BY'=>$this->usernameLogin,'UPDATED_ON'=>$this->getTime()),array('ID_ROLE' => $primary_key));
	}
    ...
    //function _after_insert_kategoripromosi($post_array){
        ...
        return $this->db->insert('mst_role',$post_array);
    }
	...
	function _after_update_role($post_array,$primary_key){
        ...
        return $this->db->update('mst_role',$post_array,array('ID_ROLE'=>$primary_key));
    }
