...
class AturUser extends CI_Controller {
    ...
    public function _aturuser()
    {
        ...
        $crud->field_type('PSSWD','invisible');
        $crud->field_type('PSSWD_DEFAULT','invisible');
      
        $crud->display_as('USERNAME','Username');
        $crud->display_as('NAMA','Nama');
        $crud->display_as('JENIS_KELAMIN','Jenis Kelamin');
        $crud->display_as('NO_HP','Nomor Handphone');
        ...
        //Atur rule
        $crud->set_rules('USERNAME','Username','trim|required|min_length[1]|max_length[50]|is_unique[MST_USER]');
        $crud->set_rules('NAMA','Nama','trim|required|min_length[1]|max_length[200]');
        $crud->set_rules('JENIS_KELAMIN','Jenis Kelamin','trim|required');
        $crud->set_rules('NO_HP','Nomor Handphone','trim|required|min_length[9]|max_length[13]|numeric');
        $crud->set_rules('EMAIL','Email','trim|required|valid_email');
        $crud->set_rules('TGL_LAHIR','Tanggal Lahir','trim|required');
        $crud->set_rules('ID_ROLE','Role','trim|required');
        ...
        $crud->field_type('JENIS_KELAMIN','dropdown',array(
            'L' => 'LAKI - LAKI', 
            'P' => 'PEREMPUAN'
            )
        );
    ...
    { 
        ...
        $this->template_lib->load('default','content/sisantuyadmin/aturuser_view',$data);
    }
    ...
    function _insert_user($post_array) 
	{
        ...
        $jenis_kelamin = $post_array['JENIS_KELAMIN'];
        $no_hp = $post_array['NO_HP'];
        $email = $post_array['EMAIL'];
        $tgl_lahir = $post_array['TGL_LAHIR'];
        $idrole = $post_array['ID_ROLE'];
        $created_by = $this->usernameLogin;

        // Untuk generate random string password
        ...
        $insertUserRole = $this->Sisantuy_Register_Model->insertUserRole(
            $iduserrole,
            $username,
            $idrole,
            $created_by
