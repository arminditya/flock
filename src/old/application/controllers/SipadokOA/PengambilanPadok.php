...
class PengambilanPadok extends CI_Controller {
	var $usernameLogin;
    ...
    {		
		if($this->session->userdata('logged_in')){
			if(page_check_authorized('Pengambilan Padok'))
			{
				$session_data = $this->session->userdata('logged_in');
				$this->usernameLogin = $session_data['username'];
                $this->_pengambilanpadok();
			}else{
	...
	public function _pengambilanpadok()
    {
        ...
        $crud->display_as('PengambilanReqTgl','Tanggal Akan Diambil');
        $crud->display_as('PengambilanReqTime','Jam Akan Diambil');
        ...
        $crud->add_action('Atur Pengambilan', '', 'demo/action_more','fa fa-check-square-o',array($this,'action_pengambilan'));
        //--- end - add action ---//
    ...
    {
		$data = array(
                'title' => 'Pengambilan Padok',
			    'body' => $output
		  );
		$this->load->helper(array('form','url'));
        $this->template_lib->load('default','content/sipadokoa/pengambilanpadok_view',$data);
    }
    ...
    //--- begin - custom action ---//
    public function action_pengambilan($primary_key , $row)
    {
        return base_url()."index.php/SipadokOA/AturPengambilan/index?KdTransaksiPadok=".$primary_key;
    }
